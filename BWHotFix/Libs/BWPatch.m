    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.09) {
        ...
        if (isReturnDouble || isReturnFloat) {
            NSMutableString *types = [NSMutableString
                                      stringWithFormat:@"%s@:", isReturnDouble ? @encode(JPDouble)
                                      : @encode(JPFloat)];
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.1) {
        ...
        if (!flag.boolValue) {
            ...
            BOOL didAddMethod = class_addMethod(
                                                self, originalSelector, method_getImplementation(swizzledMethod),
            ...
            objc_setAssociatedObject(self, JPFixedFlagKey, @YES,
...
static NSDictionary *_wrapObj(id obj) {
    ...
    return @{
        @"__obj" : obj,
        @"__clsName" : NSStringFromClass([obj isKindOfClass:[JPBoxing class]]
                                         ? [[((JPBoxing *)obj)unbox] class]
                                         : [obj class])
    };
}
...
static BOOL blockTypeIsScalarPointer(NSString *typeString) {
    ...
    NSString *typeWithoutAsterisk = trim(
...
#pragma mark 2.--设置关联属性
    ...
    [[NSBundle bundleForClass:[self class]] pathForResource:@"JSPatch"
                                                     ofType:@"js"];
    if (!path)
        _exceptionBlock(@"can't find JSPatch.js");
...
+ (JSValue *)_evaluateScript:(NSString *)script
    ...
    [self startEngine];
    if (!_regex) {
        // 使用正则表达式替换方法调用方式，将 alloc() 这样的函数调用，替换为
        // __c("alloc")() 形式
...
#pragma mark --6.js调用OC

static id callSelector(NSString *className, NSString *selectorName,
                       JSValue *arguments, JSValue *instance, BOOL isSuper) {
    // realClsName 是真正要调用的类，和 clsName
    // 的区别在于如果要调用的方法是父类方法，那么 clsName 会变为父类名，而
    // realClsName 仍然是子类名
    
    NSString *realClsName =
    [[instance valueForProperty:@"__realClsName"] toString];
    // 校验调用对象是否是类名，是否是空对象
    if (instance) {
        ...
        if (class_isMetaClass(object_getClass(instance))) {
            ...
            instance = nil;
        } else if (!instance || instance == _nilObj ||
                   [instance isKindOfClass:[JPBoxing class]]) {
            return @{ @"__isNil" : @(YES) };
        }
    }
    // 把参数列表从 JS 转到 OC
    ...
    if (isSuper) {
        ...
        if (realClsName.length) {
            ...
            // 如果 realClsName 的父类能找到就用 realClsName
            // 的父类，否则就直接用传过来的 className 转的 cls
        ...
        // 给当前的 class 添加一个 superSelector, 形式为 SUPER_XXX
        // 的形式，这个方法的指针指向父类方法
        // 之所以给子类添加一个指向父类实现的方法是因为 OC
        // 中调用父类方法调用者也是子类，是通过转发调用到父类实现的。
        // 这里就是模拟了这个过程，直接给子类添加父类实现的方法
        class_addMethod(cls, superSelector, superIMP,
                        method_getTypeEncoding(superMethod));
        // 因为是给子类动态添加的 ${SUPER_XXX}
        // 的方法，它的实现是指向父类相应的原始方法。如果父类的方法被 JS
        // 重写了，那么 ${SUPER_XXX} 也应该被 JS 重写。所以这里要对 ${SUPER_XXX}
        // 的方法进行 JS 的替换
    ...
    // 创建 NSInvocation 并且获取方法签名。其中将缓存存放在
    // _JSMethodSignatureCache 字典中的操作我认为意义不大。
    // NSInvocation 用于调用方法
    ...
    if (instance) {
        ...
        if (!methodSignature) {
            _exceptionBlock([NSString
                             stringWithFormat:@"unrecognized selector %@ for instance %@",
        ...
        if (!methodSignature) {
            _exceptionBlock(
                            [NSString stringWithFormat:@"unrecognized selector %@ for class %@",
    ...
    for (NSUInteger i = 2; i < numberOfArguments; i++) {
        ...
        switch (argumentType[0] == 'r' ? argumentType[1] : argumentType[0]) {
                // 判断是否是当前的 type，如果是，就设置 invocation 的第 i
                // 个元素设置为该类型，值为 valObj
...
break;                                                                     \
            ...
            case '{': {
                ...
                // 如果结构体是给定的类型，那么就把 js 的参数转化为该类型的结构体设置到
                // invocation 中去
...
break;                                                                     \
            ...
            default: {
                ...
                if ([(JSValue *)arguments[i - 2] hasProperty:@"__isBlock"]) {
                    ...
                    if (JPBlockClass && ![blkJSVal[@"blockObj"] isUndefined]) {
                        __autoreleasing id cb =
                        [JPBlockClass performSelector:@selector(blockWithBlockObj:)
                                           withObject:[blkJSVal[@"blockObj"] toObject]];
    ...
    /*
     最后对 OC 执行返回的结果 JS
     化。这里存在一个内存泄漏的问题。即当动态调用的方法是
     alloc，new，copy，mutableCopy 时，ARC 不会在尾部插入 release
     语句，即多了一次引用计数，需要通过 (__bridge_transfer id) 将 c 对象转为 OC
     对象，并自动释放一次引用计数，以此来达到引用计数的平衡：
     */
    char returnType[255];
...
static id (*new_msgSend1)(id, SEL, id,
                          ...) = (id(*)(id, SEL, id, ...))objc_msgSend;
static id (*new_msgSend2)(id, SEL, id, id,
                          ...) = (id(*)(id, SEL, id, id, ...))objc_msgSend;
static id (*new_msgSend3)(id, SEL, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, ...))objc_msgSend;
static id (*new_msgSend4)(id, SEL, id, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, id,
                                        ...))objc_msgSend;
static id (*new_msgSend5)(id, SEL, id, id, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, id, id,
                                        ...))objc_msgSend;
static id (*new_msgSend6)(id, SEL, id, id, id, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, id, id, id,
                                        ...))objc_msgSend;
static id (*new_msgSend7)(id, SEL, id, id, id, id, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, id, id, id, id,
                                        ...))objc_msgSend;
static id (*new_msgSend8)(id, SEL, id, id, id, id, id, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, id, id, id, id, id,
                                        ...))objc_msgSend;
static id (*new_msgSend9)(id, SEL, id, id, id, id, id, id, id, id, id,
                          ...) = (id(*)(id, SEL, id, id, id, id, id, id, id, id,
                                        id, ...))objc_msgSend;
static id (*new_msgSend10)(id, SEL, id, id, id, id, id, id, id, id, id, id,
                           ...) = (id(*)(id, SEL, id, id, id, id, id, id, id,
                                         id, id, id, ...))objc_msgSend;
// 转换参数
...
results = new_msgSend##_num(sender, selector, JP_G_ARG(0));
#define JP_CALL_MSGSEND_ARG2(_num)                                             \
results = new_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1));
...
results =                                                                    \
new_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1),            \
JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5));
...
new_msgSend##_num(sender, selector, JP_G_ARG(0), JP_G_ARG(1),            \
JP_G_ARG(2), JP_G_ARG(3), JP_G_ARG(4), JP_G_ARG(5),    \
JP_G_ARG(6), JP_G_ARG(7), JP_G_ARG(8), JP_G_ARG(9));
...
JP_CALL_MSGSEND_ARG##_realArgCount(_defineArgCount)                        \
    ...
    JP_IF_REAL_ARG_COUNT(4) {
        JP_DEAL_MSGSEND(4, 1)
        JP_DEAL_MSGSEND(4, 2) JP_DEAL_MSGSEND(4, 3) JP_DEAL_MSGSEND(4, 4)
    }
    JP_IF_REAL_ARG_COUNT(5) {
        JP_DEAL_MSGSEND(5, 1)
        JP_DEAL_MSGSEND(5, 2) JP_DEAL_MSGSEND(5, 3) JP_DEAL_MSGSEND(5, 4)
        JP_DEAL_MSGSEND(5, 5)
    }
    JP_IF_REAL_ARG_COUNT(6) {
        JP_DEAL_MSGSEND(6, 1)
        JP_DEAL_MSGSEND(6, 2) JP_DEAL_MSGSEND(6, 3) JP_DEAL_MSGSEND(6, 4)
        JP_DEAL_MSGSEND(6, 5) JP_DEAL_MSGSEND(6, 6)
    }
    JP_IF_REAL_ARG_COUNT(7) {
        JP_DEAL_MSGSEND(7, 1)
        JP_DEAL_MSGSEND(7, 2) JP_DEAL_MSGSEND(7, 3) JP_DEAL_MSGSEND(7, 4)
        JP_DEAL_MSGSEND(7, 5) JP_DEAL_MSGSEND(7, 6) JP_DEAL_MSGSEND(7, 7)
    }
    JP_IF_REAL_ARG_COUNT(8) {
        JP_DEAL_MSGSEND(8, 1)
        JP_DEAL_MSGSEND(8, 2) JP_DEAL_MSGSEND(8, 3) JP_DEAL_MSGSEND(8, 4)
        JP_DEAL_MSGSEND(8, 5) JP_DEAL_MSGSEND(8, 6) JP_DEAL_MSGSEND(8, 7)
        JP_DEAL_MSGSEND(8, 8)
    }
    JP_IF_REAL_ARG_COUNT(9) {
        JP_DEAL_MSGSEND(9, 1)
        JP_DEAL_MSGSEND(9, 2) JP_DEAL_MSGSEND(9, 3) JP_DEAL_MSGSEND(9, 4)
        JP_DEAL_MSGSEND(9, 5) JP_DEAL_MSGSEND(9, 6) JP_DEAL_MSGSEND(9, 7)
        JP_DEAL_MSGSEND(9, 8) JP_DEAL_MSGSEND(9, 9)
    }
    JP_IF_REAL_ARG_COUNT(10) {
        JP_DEAL_MSGSEND(10, 1)
        JP_DEAL_MSGSEND(10, 2) JP_DEAL_MSGSEND(10, 3) JP_DEAL_MSGSEND(10, 4)
        JP_DEAL_MSGSEND(10, 5) JP_DEAL_MSGSEND(10, 6) JP_DEAL_MSGSEND(10, 7)
        JP_DEAL_MSGSEND(10, 8) JP_DEAL_MSGSEND(10, 9)
        JP_DEAL_MSGSEND(10, 10)
    }
    return results;
}
# pragma mark -4.取出类名协议名 创建类以及给类添加协议 添加重写方法
...
static void overrideMethod(Class cls, NSString *selectorName, JSValue *function,
    ...
    if (class_getMethodImplementation(cls, @selector(forwardInvocation:)) !=
        ...
        // 为cls添加新的SEL(ORIGforwardInvocation:)，指向原始 forwardInvocation:
        // 的实现IMP.
        if (originalForwardImp) {
            class_addMethod(cls, @selector(ORIGforwardInvocation:),
                            originalForwardImp, "v@:@");
        }
    }
...
static void JPExecuteORIGForwardInvocation(id slf, SEL selector,
                                           NSInvocation *invocation) {
    ...
    if ([slf respondsToSelector:origForwardSelector]) {
        ...
        // 这里应该是保底逻辑，一般来说，不会出现调用没有 forwardInvocation
        // 方法的情况
        ...
        superForwardIMP = (void (*)(
                                    id, SEL, NSInvocation *))method_getImplementation(superForwardMethod);
        superForwardIMP(slf, @selector(forwardInvocation:), invocation);
    }
}


#pragma mark -6.消息转发 重写js实现，正对dealloc特殊处理
static void JPForwardInvocation(__unsafe_unretained id assignSlf, SEL selector,
    ...
    // （被 defineClass 过的类会被替换 forwardInvocation
    // 方法。如果有方法没有被实现，且没有被 JS重写。那么就会走原始的
    // forwardInvocation 要找到的是 ORIGforwardInvocation 方法）
    //  这是一个递归方法
    ...
    if ([slf class] == slf) {
        // 如果调用的是类方法，那么给入参列表的第一个参数就是一个包含  __clsName 的
        // object
        ...
        // 因为在 dealloc 的时候，系统不让将 self 赋值给一个 weak 对象。（在 dealloc
        // 的时候应该会有一些操作 weak 字典的步骤，所以不能再这个阶段再操作 weak）
        // assign 和 weak 的区别在于 assign 在指向的对象销毁的时候不会把当前指针置为
        // nil
...
break;                                                                     \
    ...
    // 如果当前调用的方法是 js 引起的，并且 js 调用了一个 super 的方法。那么会在
    // _currInvokeSuperClsName 中保存一个调用的方法名。这个方法名被加上了前缀
    // SUPER_。 因此真正调用的时候要把这个前缀替换为 _JP。这样才能找到保存在
    // JSOverrideMethods 字典中的相应方法
    
    if (_currInvokeSuperClsName[selectorName]) {
        Class cls = NSClassFromString(_currInvokeSuperClsName[selectorName]);
        NSString *tmpSelectorName =
        [[selectorName stringByReplacingOccurrencesOfString:@"_JPSUPER_"
                                                 withString:@"_JP"]
         stringByReplacingOccurrencesOfString:@"SUPER_"
         withString:@"_JP"];
        // 如果父类没有重写相应的方法
        if (!_JSOverideMethods[cls][tmpSelectorName]) {
            NSString *ORIGSelectorName =
            [selectorName stringByReplacingOccurrencesOfString:@"SUPER_"
                                                    withString:@"ORIG"];
            [argList removeObjectAtIndex:0];
            // 如果父类没有重写这个方法那么就是调用 oc 的方法，oc
            // 直接调用父类的相应方法
            id retObj =
            callSelector(_currInvokeSuperClsName[selectorName], ORIGSelectorName,
                         [JSValue valueWithObject:argList inContext:_context],
                         [JSValue valueWithObject:@{
                             ...
                             @"__realClsName" : @""
                                        ...
                                        inContext:_context],
                         NO);
            id __autoreleasing ret =
            formatJSToOC([JSValue valueWithObject:retObj inContext:_context]);
